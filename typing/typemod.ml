...
let type_open_ ?toplevel coerce ovf env loc lid =
...
let type_open ?toplevel env sod =
  let (path, newenv) =
    type_open_ ?toplevel None sod.popen_override env sod.popen_loc sod.popen_lid
  in
  let od =
    {
...
let () =
  ...
  Typecore.type_open := type_open_ ?toplevel:None None;
